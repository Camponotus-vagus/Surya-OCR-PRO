...
def load_image(image_input):
    ...
    if isinstance(image_input, Image.Image):
        # BOLT: Only transpose if EXIF data is present to avoid redundant image copies.
        # This is common for scanned PDFs/PNGs which often lack EXIF.
        return ImageOps.exif_transpose(image_input) if image_input.getexif() else image_input

    try:
        image = Image.open(image_input)
        
        # BOLT: Avoid redundant copy for images without EXIF
...
def load_pil_images(conversations: List[Dict[str, str]]) -> List[Image.Image]:
    ...
    for message in conversations:
        ...
        for image_path in message["images"]:
            ...
            # BOLT: Only convert if necessary to avoid redundant memory allocations
            if pil_img.mode != "RGB":
            ...
            for image, crop_shape in zip(images, images_spatial_crop):
                ...
                # with torch.inference_mode(): 
                    ...
                    if torch.sum(patches).item() != 0:
                        ...
                        # BOLT: Removed verbose print statements in performance critical path
                        # print('=====================')
...
class DeepseekOCRForCausalLM(DeepseekV2ForCausalLM):
    ...
    def infer(self, tokenizer, prompt='', image_file='', output_path = '', base_size=1024, image_size=640, crop_mode=True, test_compress=False, save_results=False, eval_mode=False):
        ...
        # BOLT: Delay expensive image copy until it's needed for visualization/saving results
        # Access original image size directly
        w, h = images[0].size
        # print(w, h)
        ...
        if '<image>' in conversation[0]['content'] and save_results:
            ...
            # print(matches_ref)
            # BOLT: Create copy only when needed for visualization
